...
Usage:
    python clip_audit.py --in results/met_metadata_enriched.csv --out-dir results --sample-per-group 150
"""
...
def stratified_sample(df, group_col="inferred_gender", n_female=150, n_male=200, seed=42):
    groups = []
...
def score_image(model, processor, image, prompts):
    """Return CLIP's softmax-normalized similarity of the image across all prompts."""
...
def bootstrap_ci(group1, group2, n_boot=1000, ci=95, seed=42):
    """Compute bootstrap CI for difference in means (group1 - group2)."""
    np.random.seed(seed)
    diffs = []
    g1 = np.array(group1)
    g2 = np.array(group2)
...
def main():
    ...
    parser.add_argument("--n-female", type=int, default=150, help="Target sample size for female artworks")
    parser.add_argument("--n-male", type=int, default=200, help="Target sample size for male artworks")
    ...
    all_prompts = list(dict.fromkeys(all_prompts))  # deduplicate preserving order

    records = []
    for _, row in tqdm(sample.iterrows(), total=len(sample), desc="Scoring CLIP images"):
        ...
        # Calculate set scores (high prompt prob - low prompt prob)
        set_scores = []
    ...
    if len(male_res) > 0 and len(female_res) > 0:
        ...
        rank_biserial = 1 - (2 * stat) / (n1 * n2)  # effect size r
        ...
        report_lines.append("Insufficient data in male or female groups for statistical evaluation.")
    ...
    # --- Plot 2: Prompt Set Robustness Comparison ---
