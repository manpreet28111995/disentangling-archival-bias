...
\author*[1]{\fnm{Manpreet} \sur{Singh}}
\email{manni@bu.edu}

\author[2]{\fnm{Rhythm}\sur{Bhatia}} \email{bhatiarhythm06@gmail.com}


\author[3]{\fnm{Rahul} \sur{Joshi}}
\email{rahulj@sitpune.edu.in}



\affil*[1]{ \orgname{Boston University}, \orgaddress{\city{Boston}, \state{MA}, \country{USA}}}
\affil[2]{ \orgname{University of Eastern Finland}, \country{Finland}}
\affil[3]{ \orgname{Symbiosis Institute of Technology, Pune, Symbiosis International (Deemed University)}, \orgaddress{\city{Pune}, \country{India}}}
...
\end{equation}
Where medium categories contain sparse representations ($N_k < 30$, such as historical sculpture in specialized sub-collections), systems must enforce an explicit fallback control: pooling sparse objects into broader structural categories (e.g., 3D/Decorative Arts) or defaulting to global corpus parameters $(\mu_{\text{global}}, \sigma_{\text{global}})$. Normalizing scores relative to physical medium baselines prevents automated discovery algorithms from systematically penalizing media historically associated with female or\subsubsection{Policy Standard 2: Mandatory Pre-Deployment Audit Workflows}
Cultural heritage institutions procuring or fine-tuning foundation models should adopt mandatory pre-deployment audit checklists before integrating them into public APIs or cataloging systems:
\begin{enumerate}
    \item \textbf{Multivariate Confound Auditing:} Institutions should fit multivariate regression models conditioning on physical medium, creation era, and image geometry to isolate demographic main effects from curation confounds.
    \item \textbf{Equivalence Testing (TOST):} Model parity should be tested via formal Two One-Sided Tests (TOST) across specified equivalence bounds ($d \le 0.30$) rather than relying on non-significant $p$-values alone.
    \item \textbf{Prompt Sensitivity Probing:} Classification systems should be evaluated across multiple semantic prompt pairs, checking whether craft or decorative descriptors induce asymmetric score shifts.
\end{enumerate}

\subsubsection{Policy Standard 3: Metadata Provenance Transparency and Anonymity Labeling}
Automated tagging and AI-assisted cataloging should preserve metadata provenance \cite{mitchell2019model, dignum2019responsible}. Algorithmic tags need to be visually separated from human curatorial attributions, with confidence metrics and model version metadata attached. To counter survival bias in digital archives, public interfaces should mark unattributed or anonymized holdings and draw attention to domestic craft and textile collections so that AI search indexing does not bury historically anonymized labor.
...
The scope of this audit is bounded by several methodological and architectural constraints:

\subsubsection{Architectural Scaling Dynamics: ViT-B/32 vs. ViT-L/14}
We tested dual-encoder ViT-B/32 backbones ($86\text{M}$ vision parameters, $63\text{M}$ text parameters, joint embedding dimension $D=512$). Larger architectures, for instance ViT-L/14 ($304\text{M}$ vision parameters, $D=768$) and ViT-H/14 ($632\text{M}$ vision parameters, $D=1024$), have higher feature capacity, finer patch resolution ($14 \times 14$ vs. $32 \times 32$), and higher learned logit scale parameters $\tau$. Larger encoders learn sharper visual feature representations that may cut residual embedding noise, possibly raising sensitivity to subtle visual surface attributes. On the other hand, parameter scaling also makes models more susceptible to memorizing pretraining data skews, since larger encoders absorb web-scraped associations more tightly. Future audits should benchmark zero-shot valuation stability across parameter scales ($86\text{M} \to 632\text{M}$) to test whether increased capacity changes the score noise floor.

\subsubsection{Contrastive Softmax Loss vs. Pairwise Sigmoid Loss (SigLIP)}
Standard OpenAI CLIP and OpenCLIP architectures optimize a symmetric contrastive softmax loss (Equation 1), which normalizes image-text similarity scores across all candidate text prompts concurrently within a batch. As shown in Equation 2, computing zero-shot probabilities via softmax renders logit differential scores sensitive to the temperature parameter $\tau$ and the candidate prompt baseline cardinality $K$. In contrast, recent architectures such as SigLIP \cite{zhai2023sigmoid} replace softmax normalization with a pairwise sigmoid loss that operates independently on image-text pairs. Decoupling prompt probability estimation from batch-wide normalization avoids scale compression and temperature distortion. Testing SigLIP on cultural heritage collections is a natural next step to check whether pairwise loss improves score calibration for long-tail art categories.

\subsubsection{Generative Multimodal LLMs (MLLMs) and Open-Ended Evaluation}
Zero-shot contrastive dual-encoders compute static visual-text cosine similarities. By contrast, instruction-tuned Multimodal Large Language Models (MLLMs) like LLaVA, InstructBLIP, and Qwen-VL evaluate visual art through autoregressive text generation. These models can produce natural language rationales for aesthetic judgment, catalog description, and historical context. They also inherit hallucination patterns, text-based reasoning biases, and instruction-following artifacts from their language model backbones. Auditing MLLMs in museum archives would require moving beyond contrastive logit scoring toward generative evaluation frameworks that combine NLP quality metrics with human expert review.

\subsubsection{Single-Institution Scope and Archival Generalizability}
This audit covers a single encyclopedic museum corpus ($N=1,500$ objects from the Metropolitan Museum of Art), which reflects the specific acquisition trajectories, cataloging conventions, and digitization standards of one major Anglo-American institution. Dictionary-based gender resolution excluded $41.2\%$ ($n=618$) unattributed objects, showing how legacy metadata gaps produce dataset survival bias. Broad semantic prestige categories (\textit{masterpiece}, \textit{quality}, \textit{influence}) also sit near an embedding noise floor and cannot detect localized visual feature biases (e.g., subject gaze or compositional framing).
...
We audited zero-shot VLM valuations across Metropolitan Museum of Art Open Access metadata ($N=1,500$ total records; $N=743$ attributed named works: Male $n=534$, Female $n=209$; $n=618$ anonymous/unattributed), testing whether observed score differences reflect direct demographic bias or archival confounders. Under OpenAI CLIP, no significant gender disparity appeared ($U = 59,307.00, p = 0.1829, r = -0.0628$); OpenCLIP showed a consistent pattern ($U = 59,867.00, p = 0.1224, r = -0.0728$). TOST confirmed equivalence at Cohen's $d \ge 0.25$ ($p_{\text{TOST}} = 0.0042$ at $d=0.30$ for OpenAI CLIP; $p_{\text{TOST}} = 0.0024$ for OpenCLIP), with sensitivity checks spanning $d \in [0.15, 0.40]$. Multivariate OLS regression showed non-significant conditional effects for artist gender ($B = 0.0037, p = 0.202$ for CLIP; $B = 0.0065, p = 0.356$ for OpenCLIP) and aspect ratio ($B = -0.0064, p = 0.141$ for CLIP; $B = -0.0023, p = 0.516$ for OpenCLIP), with overall $R^2 < 2\%$ pointing to an embedding noise floor.

Two points deserve emphasis: (i) macro-level zero-shot score equivalence does not rule out localized visual feature biases at finer granularity, and (ii) the exclusion of $41.2\%$ ($n=618$) unattributed holdings reflects an institutional survival bias that cannot be eliminated, since named creators are themselves an already-filtered subset of historical acquisition decisions. As cultural heritage institutions adopt VLMs for collection indexing and public discovery, medium-stratified z-score normalization with fallback controls and archival provenance standards become necessary to prevent automated systems from perpetuating historical representational gaps. Multivariate confound controls and pretraining curation audits are needed to support fair AI deployment across global cultural repositories.
...
This appendix supplements the regression results with qualitative case comparisons that show how photographic framing artifacts and prompt-level taxonomic choices affect model outputs.
...
Prompt Set 3 probes artistic status by calculating probability differentials between "a groundbreaking artwork" ($\mathbf{p}_{\text{high}}^{(3)}$) and "a decorative craft object" ($\mathbf{p}_{\text{low}}^{(3)}$). Robustness checks across all $N=743$ attributed works confirm high evaluation stability across prompt pairs ($p > 0.10$), indicating that prompt phrasing does not induce systematic demographic valuation shifts.
\end{appendices}
